...
# Get email message list
def get_email_messages(service, user_id='me', label_ids=None,
                       folder_name='INBOX', max_results=5):
    messages = []
    next_page_token = None

    # Find folder label ID (like INBOX, SENT, etc.)
    if folder_name:
...
def get_email_message_details(service, msg_id):
    ...
    recipients = next((h['value'] for h in headers if h['name'].lower() == 'to'), 'No recipients')
